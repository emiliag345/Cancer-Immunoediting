...
At t=0, only a single clone exists. As time goes on, clones were randomly spawned by setting ![x1](https://latex.codecogs.com/gif.latex?x_i) to 1 with a probability of 0.05 per unit time. A clone was marked eliminated (![x1](https://latex.codecogs.com/gif.latex?x_i) = 0) if ![x1](https://latex.codecogs.com/gif.latex?x_i) fell below 0.1. This was to prevent a resurgence of infinitesimal traces of cells over time.
